...
public class AppointmentBookingTest {
    ...
    public void bookAppointmentTest() {
        ...
        ((JavascriptExecutor) driver).executeScript("arguments[0].value='15/05/2026';", date);
        driver.findElement(By.id("txt_comment")).sendKeys("Regular checkup");
        ...
        WebElement confirmationHeading = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//section[@id='summary']//h2")));
    ...
    public void verifyAppointmentDetailsTest() {
        ...
        WebElement date = wait.until(ExpectedConditions.elementToBeClickable(By.id("txt_visit_date")));
        date.click();
        date.sendKeys(Keys.chord(Keys.CONTROL, "a"));
        date.sendKeys(Keys.DELETE);
        ((JavascriptExecutor) driver).executeScript("arguments[0].value='25/05/2026';", date);
        driver.findElement(By.id("txt_comment")).sendKeys("Dental appointment");
        ...
        String actualDate = driver.findElement(By.id("visit_date")).getText();
    ...
    public void hospitalAdmissionCheckboxTest() {
        ...
        WebElement date = driver.findElement(By.id("txt_visit_date"));
        ((JavascriptExecutor) driver).executeScript("arguments[0].value='30/05/2026';", date);
        driver.findElement(By.id("txt_comment")).sendKeys("Need admission");
        WebElement submitButton = driver.findElement(By.id("btn-book-appointment"));
        ((JavascriptExecutor) driver).executeScript("arguments[0].click();", submitButton);
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("summary")));
        String admissionStatus = driver.findElement(By.id("hospital_readmission")).getText();
        Assert.assertEquals(admissionStatus, "Yes");
    }
    @Test(priority = 4)
    public void rejectPastDateTest() {
        login();
        WebElement dateField = wait.until(ExpectedConditions.elementToBeClickable(By.id("txt_visit_date")));
        ((JavascriptExecutor) driver).executeScript("arguments[0].value='01/01/2020';", dateField);
